...
static void shiftreg_set(uint16_t sreg) {
	for (signed int i = 15; i >= 0; i--) {
		if ((sreg >> i) & 1) {
			PORT_DATA |= (1 << DBG_DATA);
		} else {
			PORT_DATA &= ~(1 << DBG_DATA);
		}

		PORT_SHIFT |= (1 << DBG_SHIFT);
		PORT_SHIFT &= ~(1 << DBG_SHIFT);
	}

	PORT_SYNC |= (1 << DBG_SYNC);
	PORT_SYNC &= ~(1 << DBG_SYNC);
}
...
void lcd_setcursor(uint8_t wat, uint8_t wat2) {
}
